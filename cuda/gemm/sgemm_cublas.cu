...
void CublasSgemm(float* a, float* b, float* c, const int M, const int N, const int K) {
  float alpha = 1.0;
  float beta = 0.0;
...
int main() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    printf("Cublas Accuracy Test %d passed, M = %d, N = %d, K = %d, max_diff: %f.\n", i, M_list[i], N_list[i],
           K_list[i], max_diff);
  }
  ...
  for (int i = 0; i < 14; i++) {
    ...
    printf(
      "Cublas Performance Test %d, M = %d, N = %d, K = %d, time = %.6f %.6f %.6f sec, gflops = %.6f "
      "GFLOPS\n",
